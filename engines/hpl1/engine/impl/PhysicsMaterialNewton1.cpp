...
namespace hpl {
										...
										const NewtonContact* apContact)
	{
		...
		//Force
		float vForce[3];
		NewtonMaterialGetContactForce(apMaterial,vForce);
		mContactData.mvForce += cVector3f::fromArray(vForce);

		//Position and normal
		float matPos[3], matNormal[3];
		NewtonMaterialGetContactPositionAndNormal(apMaterial,matPos, matNormal);

		mContactData.mvContactNormal += cVector3f::fromArray(matNormal);
		mContactData.mvContactPosition += cVector3f::fromArray(matPos);
		...
		if(mpContactBody1->GetWorld()->GetSaveContactPoints())
		{
			...
			NewtonMaterialGetContactPositionAndNormal (apMaterial, matPos,
														matNormal);
